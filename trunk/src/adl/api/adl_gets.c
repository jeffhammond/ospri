...
#   endif
    ...
    if(stride_level == 0)
    {
        ...
        {
           status = OSPU_Get_memcpy(source_ptr, target_ptr, block_sizes[0]);
    ...
    else /* Non-contiguous */
    {
        ...
        {
            status = OSPU_GetS_memcpy(stride_level,
                                     block_sizes,
                                     source_ptr,
                                     src_stride_ar,
                                     target_ptr,
                                     trg_stride_ar);
            OSPU_ERR_POP(status!=OSP_SUCCESS, "OSPU_GetS_memcpy returned error\n");
        }
        else
        {
