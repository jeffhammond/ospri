...
int main(int argc, char* argv[])
{
    ...
    if (world_rank==0)
    {
        ...
        if (env_char!=NULL)
        {
            /* use binary rather than decimal since the former is likely
             * to be an integer multiple of the page size                */
            if      ( NULL != strstr(env_char,"G") ) units = 1024*1024*1024;
            else if ( NULL != strstr(env_char,"M") ) units = 1024*1024;
            else if ( NULL != strstr(env_char,"K") ) units = 1024;
            else                                     units = 1;
