...
int PAMID_Get_endtoend(size_t bytes, void * local, size_t target, void * remote)
{
	...
	result = PAMI_Get(PAMID_INTERNAL_STATE.pami_contexts[PAMID_INTERNAL_STATE.context_roles.local_blocking_context], &parameters);
	PAMID_ASSERT(result==PAMI_SUCCESS,"PAMI_Get");
...
int PAMID_Get_nonblocking(size_t bytes, void * local, size_t target, void * remote, pamid_request_t * request)
{
	...
	result = PAMI_Get(PAMID_INTERNAL_STATE.pami_contexts[PAMID_INTERNAL_STATE.context_roles.local_offload_context], &parameters);
	PAMID_ASSERT(result==PAMI_SUCCESS,"PAMI_Get");
