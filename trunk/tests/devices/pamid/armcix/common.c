...
/* --------------------------------------------------------------- */
void dispatch_contig(pami_context_t    context,     /**< IN: PAMI context */
  ...
  if (recv)
  {
    if (ainfo->scale != 1.0) 
    {
      ...
      *ainfo            = *(ARMCIX_Info_t *)header_addr;
      recv->data_fn     = qpx_data_fn; /* called on every packet */
  ...
  pami_recv_t        * recv )       /**< OUT: receive message structure */
{
  ...
  //  ainfo->scale, ainfo->dstaddr);    
  if (recv) /* need to recv subsequent packets */
  {
    ainfo = (ARMCIX_Info_t *)malloc(header_size);
    *ainfo = *(ARMCIX_Info_t *)header_addr;
    recv->local_fn = cb_recv_done;
  ...
  else  /* short protocol, data available in first packet */
  {
