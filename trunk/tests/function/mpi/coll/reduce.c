...
void reduce_only(FILE * output, MPI_Comm comm, int max_mem)
{
	...
	for (int c=1; c<max_count; c*=2)
	{
		...
		if ( comm_rank == root )
			fprintf(output, "MPI_Reduce %d integers in %lf seconds (%lf MB/s) \n",
	...
	for (int c=1; c<max_count; c*=2)
	{
		...
		if ( comm_rank == root )
			fprintf(output, "MPI_Reduce %d doubles in %lf seconds (%lf MB/s) \n",
					c, dt_red, 1.0e-6*c*sizeof(double)/dt_red );
