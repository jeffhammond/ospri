...
/**
 * \file test/api/p2p/rput/rput_perf.c
 * \brief PAMI_Rput() latency test
 **/
  ...
  if (me == 0)
    {
      ...
      fprintf (stdout, "#\n");
      fprintf (stdout, "# PAMI_Rput() blocking latency performance test\n");
      fprintf (stdout, "#\n");
      fprintf (stdout, "# origin task 0 invokes PAMI_Rput().\n");
      ...
      for (; nbytes <= BUFFERSIZE; nbytes = nbytes * 2)
        {
          char str[10240];
          int index = 0;
          ...
          for (n = 0; n < ntargets; n++)
            {
...
#endif
              /* Calculate the ping-pong rput latency. The number of cycles to
               * perform a half ping-pong rput will be subtracted from the
               * aggregate number of cycles to perform the rput data transfer */
              parameters.rma.bytes    = 1;
              ...
              unsigned long long half_pingpong_cycles = ((t1 - t0) / ITERATIONS) / 2;
              /*fprintf (stderr, "target task: %d, half_pingpong_cycles = %llu\n", target[n].task, half_pingpong_cycles);*/
              ...
              cycles = ((t1 - t0) / ITERATIONS) - half_pingpong_cycles;
              usec   = cycles * tick * 1000000.0;
              index += sprintf (&str[index], "%7.4f %8.6f", usec, nbytes*1000000.0/(1024*1024*usec));
            }
