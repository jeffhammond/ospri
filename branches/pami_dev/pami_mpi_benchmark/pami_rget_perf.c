...
#define BUFFERSIZE 32*1024*1024
	...
	if (me == 0)
	{
		...
		for (; nbytes <= BUFFERSIZE; nbytes = nbytes * 2)
		{
			...
			for (n = 0; n < ntargets; n++)
			{
				...
				usec   = cycles * tick * 1000000.0*2;
				index += sprintf (&str[index], "%7.4f %8.4f  ", usec, nbytes*1000000/(1024*1024*usec));
			}
