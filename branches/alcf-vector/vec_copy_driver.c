...
int main(int argc, char* argv[])
{
    ...
    printf("vec_copy test for %lu elements \n", n);
    ...
    for (unsigned int r=0;r<warmup;r++) 
        vec_copy_c_basic(n,x,y);

    t0 = getticks();
    for (unsigned int r=0;r<repetitions;r++) 
        vec_copy_c_basic(n,x,y);
    t1 = getticks();
    printf("%20s of %lu %s took %12llu cycles (%6.2lf cycles/element) \n", "vec_copy_c_basic", n, REALNAME , (t1-t0)/repetitions, ((double)t1-t0)/(repetitions*n) );

    /* unroll2 */

    for (unsigned int r=0;r<warmup;r++) 
        vec_copy_c_unroll2(n,x,y);

    t0 = getticks();
    for (unsigned int r=0;r<repetitions;r++) 
        vec_copy_c_unroll2(n,x,y);
    t1 = getticks();
    printf("%20s of %lu %s took %12llu cycles (%6.2lf cycles/element) \n", "vec_copy_c_unroll2", n, REALNAME , (t1-t0)/repetitions, ((double)t1-t0)/(repetitions*n) );

    /* unroll4 */

    for (unsigned int r=0;r<warmup;r++) 
        vec_copy_c_unroll4(n,x,y);

    t0 = getticks();
    for (unsigned int r=0;r<repetitions;r++) 
        vec_copy_c_unroll4(n,x,y);
    t1 = getticks();
    printf("%20s of %lu %s took %12llu cycles (%6.2lf cycles/element) \n", "vec_copy_c_unroll4", n, REALNAME , (t1-t0)/repetitions, ((double)t1-t0)/(repetitions*n) );

    /* unroll4 */

    for (unsigned int r=0;r<warmup;r++) 
        vec_copy_c_unroll8(n,x,y);

    t0 = getticks();
    for (unsigned int r=0;r<repetitions;r++) 
        vec_copy_c_unroll8(n,x,y);
    t1 = getticks();
    printf("%20s of %lu %s took %12llu cycles (%6.2lf cycles/element) \n", "vec_copy_c_unroll8", n, REALNAME , (t1-t0)/repetitions, ((double)t1-t0)/(repetitions*n) );
